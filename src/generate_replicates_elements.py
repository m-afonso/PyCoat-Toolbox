...
def generate_replicates_elements(
    ...
    _mean_entry = tk.Entry(
        interface_window,
        state='readonly',
        justify='center')

    _mean_entry.grid(row=_initial_row + len(_replicates_list) + 1, column=_initial_column + 4, padx=padx_value)

    return _replicates_list, _mean_entry


def get_values_replicates_elements(replicates_elements_list: List[Dict[str, tk.Entry | tk.Label]],
