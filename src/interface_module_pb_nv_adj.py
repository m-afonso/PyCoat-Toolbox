...
def pb_nv_adj_interface_window(master_window=None):
    ...
    frames = {
        'target': BathFrame(interface_pb_nv_adj_window,
                            'Target Bath',
                            initial_row,
                            initial_col,
                            ('Weight', 'NV%', 'P/B%'),
                            resin_pigment_frame=False),
        'initial': BathFrame(interface_pb_nv_adj_window,
                             'Initial Bath',
                             initial_row,
                             initial_col + 2,
                             ('Weight', 'NV%', 'P/B%'),
                             ...
                             resin_pigment_frame=False),
        'paste': BathFrame(interface_pb_nv_adj_window,
                           'Paste',
                           initial_row,
                           initial_col + 4,
                           ('Weight', 'Binder%', 'Pigment%'),
                           elements_states=('readonly', 'normal', 'normal')),
        'resin': BathFrame(interface_pb_nv_adj_window,
                           'Resin',
                           initial_row,
                           initial_col + 6,
                           ('Weight', 'Binder%', 'Pigment%'),
                           elements_states=('readonly', 'normal', 'disabled')),
        'water': BathFrame(interface_pb_nv_adj_window,
                           'Water',
                           initial_row,
                           initial_col + 8,
                           ('Weight', 'Binder%', 'Pigment%'),
    ...
    # Botões
    run_button = tk.Button(interface_pb_nv_adj_window, text="Run", command=lambda: run(frames), width=15)
    run_button.grid(row=initial_row + 1, column=initial_col, columnspan=1, pady=25, padx=10)

    # procedure widget
    procedure_path = "procedures/material_balance.txt"
    procedure_textbox(interface_pb_nv_adj_window,
                      _row=initial_row + 2,
                      _column=0,
...
class BathFrame:
    def __init__(self,
        ...
        self.frame = tk.Frame(_root_window, padx=10, pady=10, borderwidth=2, relief="solid")
        self.frame.grid(row=frame_row, column=frame_col, padx=10, pady=10)

        frame_label = tk.Label(self.frame, text=frame_title, font=("Helvetica", 12, "bold"))
    ...
    def get_bath(self) -> Bath | None:
        ...
        if self.resin_pigment_frame:
            ...
            bath.binder = float(self.elements[1]['entry'].get()) / 100  # convert binder% to binder (decimal)
            bath.pigment = float(self.elements[2]['entry'].get()) / 100  # convert pigment% to pigment (decimal)
        else:
            bath = Bath(float(self.elements[0]['entry'].get()),
                        bath_nv=float(self.elements[1]['entry'].get()) / 100,  # convert NV% to NV (decimal)
                        bath_pb=float(self.elements[2]['entry'].get()) / 100)  # convert P/B% to P/B (decimal)
...
def run(frames: Dict[str, BathFrame]):
    try:
        _results = adjust_bath(frames['target'].get_bath(),
...
def adjust_bath(target_bath: Bath,
                ...
                paste_min: int | float = 0) -> Dict[str, Bath]:
